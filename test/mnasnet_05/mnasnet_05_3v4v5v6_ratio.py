...
def build_block_df(b_data, l_data):
    """按 Block 边界聚合 Layer 数据，计算每个 block 的通信占比"""
    ...
    for b in b_data:
        b_idx = set(b['indices'])
        tot = sum(l_tot_map[i] for i in b_idx if i in l_tot_map)
        comp = sum(l_comp_map[i] for i in b_idx if i in l_comp_map)
        comm = tot - comp
        rows.append({
            'name': b['name'],
            'range': b['range'],
            ...
            'compute': comp,
            'comm': comm,
            'comm_pct': comm / tot * 100 if tot > 0 else 0,
        })
...
for n_mcu, folder in [(3, '3_mcus'), (4, '4_mcus'), (5, '5_mcus'), (6, '6_mcus')]:
    b = parse_log(f'./test/{folder}/coordinator_mnasnet_pw_block.log')
    l = parse_log(f'./test/{folder}/coordinator_mnasnet_pw_layer.log')
    dfs[n_mcu] = build_block_df(b, l)
...
ax.set_ylim(0, 100)
ax.set_title('MobileNetV2-0.35 Layer Mode: Communication Ratio',
...
plt.tight_layout()
plt.savefig('./test/mnasnet_05/mnasnet_05_3v4v5v6_ratio.png', dpi=300)
# print("Saved to ./test/mnasnet_05_4v5_ratio.png")
